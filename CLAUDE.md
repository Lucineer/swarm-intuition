# CLAUDE.md — Swarm Intuition

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Swarm Intuition
- **Role**: Swarm Intuition
- **URL**: https://swarm-intuition.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/swarm-intuition
- **Branch**: master
...
### Deploy
```bash
cd /tmp/swarm-intuition && wrangler deploy
```

### Health Check
```bash
curl -s https://swarm-intuition.casey-digennaro.workers.dev/health
curl -s https://swarm-intuition.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/predictions | API endpoint |
| /api/signal | API endpoint |
| /api/signals | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=swarm-intuition
...
- **vessel.json capabilities**: pattern-prediction, bottleneck-forecast
...
1. Keep the vessel small. ~165 lines is the current size.
